...
var _Sources = (() => {
  ...
  var WebtoonParser = class {
    ...
    parseDetailsThumbnail($) {
      const thumb = $("#wrap > #container > #content > div.detail_bg").attr("style")?.match(/url\('(.*?)'\)/)?.[1] ?? "";
    ...
    parsePopularTitles($) {
      return $("div#content div.webtoon_list_wrap ul.webtoon_list li a").toArray().filter((elem) => $(elem).find("strong.title")).map((elem) => this.parseMangaFromElement($(elem)));
    }
    parseTodayTitles($, allTitles) {
      ...
      for (let i = 0; i <= list.length - 1 && (allTitles || mangas.length < 10); i++) {
    ...
    */
    ...
    parseSearchResults($, canvas_wanted) {
      const items = [];
      items.push(...$("#content > div.webtoon_list_wrap ul li a._card_item").toArray().map((elem) => this.parseMangaFromElement($(elem))));
      if (canvas_wanted) {
    ...
    parseGenres($) {
      return $("#content > div#genre_wrap > div.snb_inner > ul li").toArray().map((elem) => this.parseTagFromElement($(elem)));
    }
    parseCanvasGenres($) {
      return $("#content > div#genre_wrap > div.snb_inner > ul li").toArray().filter((elem) => $(elem).attr("data-genre") && $(elem).attr("data-genre") !== "ALL").map((elem) => this.parseCanvasTagFromElement($(elem)));
    }
    ...
    parseTagResults($) {
      return App.createPagedResults({
        results: $("#content > div.webtoon_list_wrap ul li a").toArray().map((elem) => this.parseMangaFromElement($(elem)))
      });
    }
  };
  ...
  var Webtoon = class {
    constructor(cheerio, LOCALE2, BASE_URL2, MOBILE_URL2, HAVE_TRENDING2) {
      this.cheerio = cheerio;
      this.LOCALE = LOCALE2;
      this.BASE_URL = BASE_URL2;
      ...
      this.parser = new WebtoonParser(LOCALE2, BASE_URL2);
      this.cookies = [
  ...
  var WebtoonEN = class extends Webtoon {
    constructor(cheerio) {
      super(cheerio, LOCALE, BASE_URL, MOBILE_URL, HAVE_TRENDING);
    }
  };
